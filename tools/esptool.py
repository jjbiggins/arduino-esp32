...
class ESP32C2FirmwareImage(ESP32FirmwareImage):
    ...
    def set_mmu_page_size(self, size):
        if size not in [16384, 32768, 65536]:
            raise FatalError(f"{size} is not a valid page size.")
    ...
    while True:
        ...
        if read_bytes == b'':
            if partial_packet is None:  # fail due to no data
                ...
                msg = f"Packet content transfer stopped (received {len(partial_packet)} bytes)"
            trace_function(msg)
        ...
        for b in read_bytes:
            ...
            if partial_packet is None:  # waiting for packet header
                if b == b'\xc0':
                    ...
                    raise FatalError(
                        f'Invalid head of packet (0x{hexify(b)}): Possible serial noise or corruption.'
                    )
...
class FatalError(RuntimeError):
    ...
    def WithResult(message, result):
        ...
        message += f" (result was {hexify(result)}: {err_defs.get(err_code[0], 'Unknown result')})"
